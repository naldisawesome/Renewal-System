...
export default async function AdminOverview() {
  ...
  const [total, unassigned, inProgress, renewed, pendingUsers, byCompany, byMonth] = await Promise.all([
    ...
    prisma.renewal.count({ where: { status: { in: ["ASSIGNED", "IN_PROGRESS", "QUOTED", "CONTACTED"] } } }),
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">July renewal review — Cactus &amp; Blanket</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {cards.map((c) => (
            <Link key={c.label} href={c.href} className="card p-5 hover:shadow-md transition-shadow">
              <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">{c.value}</div>
        ...
        <div className="card p-5">
          <h2 className="font-medium text-slate-900 dark:text-slate-100 mb-3">By Type</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {byCompany.map((row) => (
              <div key={`${row.company}-${row.bookType}`} className="rounded-md border border-slate-200 dark:border-slate-700 p-4">
            ...
            {byCompany.length === 0 && (
              <p className="text-sm text-slate-400 dark:text-slate-500">No renewals uploaded yet.</p>
            )}
          </div>
        </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-medium text-slate-900 dark:text-slate-100">By Renewal Month</h2>
            <span className="text-xs text-slate-400 dark:text-slate-500">Based on each renewal's Renewal Date</span>
          </div>
          <div className="flex flex-wrap gap-3">
            {byMonth.map((m) => (
              <Link
                ...
                className="rounded-md border border-slate-200 dark:border-slate-700 px-4 py-3 hover:border-brand-300 hover:bg-brand-50 dark:hover:bg-brand-900/30 transition-colors min-w-[110px]"
              >
                <div className="text-lg font-semibold text-slate-900 dark:text-slate-100">{m.count}</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">{m.label}</div>
              </Link>
            ))}
            {byMonth.length === 0 && (
              <p className="text-sm text-slate-400 dark:text-slate-500">No renewal dates to group yet.</p>
            )}
          </div>
        </div>
