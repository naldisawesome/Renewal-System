...
export async function POST(req: Request) {
  ...
  // Defense in depth: cap the upload size before reading it into memory.
  // Legitimate renewal exports are a few MB at most - this just stops an
  // accidental (or malicious) huge file from spiking the function's memory.
  const MAX_UPLOAD_BYTES = 20 * 1024 * 1024; // 20MB
  if (file.size > MAX_UPLOAD_BYTES) {
    return NextResponse.json(
      { error: "That file is too large (max 20MB). Split it into smaller batches and try again." },
      { status: 413 }
    );
  }
