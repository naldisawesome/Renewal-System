...
import { CompanyBadge, BookTypeBadge } from "@/components/Badges";
...
export default async function RenewalDetailPage({ params }: { params: { id: string } }) {
  ...
  // Ownership is role-specific: an Underwriter can only act on renewals
  // where they're the assigned underwriter, an Adviser only where they're
  // the assigned adviser (mirrors the check in the status PATCH route).
  const isOwner =
    (session.user.role === "ADVISER" && renewal.assignedAdviserId === session.user.id) ||
    (session.user.role === "UNDERWRITER" && renewal.assignedUnderwriterId === session.user.id);
  ...
  return (
    <div>
      <NavBar role={session.user.role} name={session.user.name} email={session.user.email} />
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{renewal.clientName}</h1>
        ...
        <div className="card p-5 space-y-4">
          <div className="flex gap-8">
            <div>
              <div className="text-sm text-slate-500 dark:text-slate-400">Adviser</div>
              <div className="font-medium text-slate-900 dark:text-slate-100">{renewal.assignedAdviser?.name || "Unassigned"}</div>
            </div>
            <div>
              <div className="text-sm text-slate-500 dark:text-slate-400">Underwriter</div>
          ...
          {canChangeStatus ? (
            <StatusControl
              ...
              statusSince={renewal.updatedAt.toISOString()}
              role={session.user.role}
            />
          ) : (
            <div>
              <div className="label">Status</div>
              <RenewalProgress currentStatus={renewal.status} statusSince={renewal.updatedAt} />
            </div>
          )}
        </div>
