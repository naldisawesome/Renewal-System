      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 card shadow-lg z-50 max-h-96 overflow-y-auto">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700">
            <span className="font-medium text-sm text-slate-900 dark:text-slate-100">Notifications</span>
            {unreadCount > 0 && (
          ...
          <div className="divide-y divide-slate-100">
            {notifications.map((n) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800 ${!n.read ? "bg-brand-50 dark:bg-brand-900/30/50" : ""}`}
              >
            ...
            {notifications.length === 0 && (
              <p className="px-4 py-6 text-sm text-slate-400 dark:text-slate-500 text-center">No notifications yet.</p>
            )}
          </div>
        </div>
      )}
