...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-md shadow-sm px-3 py-2 text-sm">
...
export default function TrendChart({ data }: { data: { label: string; total: number; count: number }[] }) {
  if (data.length === 0) {
    return <p className="text-sm text-slate-400 py-16 text-center">No dated policies to chart yet.</p>;
  }
